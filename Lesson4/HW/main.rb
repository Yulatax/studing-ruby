...
class MainInterface
  ...
  def main
    loop do
      ...
      case choice
      when 1
        create_instance('to create station', method(:create_station))
      when 2
        create_instance('to create train', method(:create_train))
      when 3
        create_instance('to create route', method(:create_route))
      when 4
        create_instance('to create train car', method(:create_train_car))
      when 5
        create_instance('to add station to route', method(:add_station_to_route))
      when 6
        create_instance('to delete station from route', method(:delete_station_from_route))
      when 7
        create_instance('to set route to train', method(:set_route_to_train))
      when 8
        create_instance('to add/remove train car', method(:manage_train_car))
      when 9
        create_instance('to move train', method(:move_train))
      when 10
        create_instance('to display stations list', method(:display_stations_list))
      when 11
        create_instance('to display trains on station', method(:display_trains_on_station))
      else
  ...
  def to_string
    @railway.each_pair do |key, value|
      output = "#{key.to_s}:\n"
      case key.to_s
      when 'stations'
        value.each { |station| output += "\t#{station.name}\n" }
      when 'trains'
        value.each { |train| output += "\t#{train.number}: #{train.type}\n" }
      when 'routes'
        value.each { |route| output += "\t#{route.number}: #{route.start_station.name} - #{route.end_station.name}\n" }
      when 'cars'
        value.each { |car| output += "\t#{car.number}: #{car.type}\n" }
      else
        output = "Error!"
      end
      puts output
  ...
  def station_exist?(name)
    @railway[:stations].any?{ |station| station.name == name }
  end

  def train_exist?(number)
    @railway[:trains].any?{ |train| train.number == number }
  end

  def route_exist?(number)
    @railway[:routes].any?{ |route| route.number == number }
  end

  def car_exist?(number)
    @railway[:cars].any?{ |car| car.number == number}
  end

  def find_station(name)
    @railway[:stations].find{ |station| station.name == name }
  end

  def find_route(number)
    @railway[:routes].find{ |route| route.number == number }
  end

  def find_train(number)
    @railway[:trains].find{ |train| train.number == number }
  end

  def find_train_car(number)
    @railway[:cars].find{ |car| car.number == number }
  end
  ...
  def display_stations_list
    @railway[:stations].each {|station| puts station.name}
  end
